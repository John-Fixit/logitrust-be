 ...
 *
 * **Route discovery:** run `npm run openapi:generate` to refresh `openapi.autogen.json`
 * from `src/app.js` (swagger-autogen). At runtime, autogen paths are merged first, then
 * `staticPaths` (rich request/response docs), then registry CRUD from `buildResourcePaths()`.
 *
 * Dynamic `forEach` mounts (e.g. `/api/resources/...`) are not detected by swagger-autogen;
 * those stay generated from `src/modules/resource-registry.js`.
 */
const fs = require("fs");
...
function normalizeOpenApiPathKey(routePath) {
...
/** Paths from `npm run openapi:generate` (optional file). */
function loadAutogeneratedPaths() {
  const filePath = path.join(__dirname, "openapi.autogen.json");
  try {
    const raw = JSON.parse(fs.readFileSync(filePath, "utf8"));
    const paths = raw.paths || {};
    const out = {};
    for (const [key, spec] of Object.entries(paths)) {
      if (key === "/api-docs.json" || key.startsWith("/docs")) continue;
      out[normalizeOpenApiPathKey(key)] = spec;
    }
    return out;
  } catch {
    return {};
...
/** Merge OpenAPI `paths`: same path key merges HTTP methods; `overlay` wins on conflicts. */
function mergePathSpecs(baseline, overlay) {
  const keys = new Set([...Object.keys(baseline), ...Object.keys(overlay)]);
  const merged = {};
  for (const k of keys) {
    const b = baseline[k] || {};
    const o = overlay[k] || {};
...
function getOpenApiSpec() {
  ...
  const staticPaths = {
    ...
    "/api/user/{id}": {
      ...
      delete: {
        ...
        description:
          "Permanently deletes user with the given id. The path `{id}` must equal the JWT `sub` (same numeric user id as in your token).",
        security: bearerSecurity,
        parameters: [{ $ref: "#/components/parameters/IdPath" }],
        responses: {
          200: {
      ...
      post: {
        ...
        requestBody: {
          ...
          content: {
            "application/json": {
              schema: {
                $ref: "#/components/schemas/ClerkGoogleExchangeRequest",
              },
            },
          },
        },
  ...
  return {
    ...
    tags: [
      ...
      {
        name: "Auth",
        description: "Registration, login, email verification, Clerk",
      },
      { name: "User", description: "User profiles (admin list + user by id)" },
      {
        name: "Shipments",
        description: "Shipment flows (tracking code, status)",
      },
    ...
    paths: mergePathSpecs(
      mergePathSpecs(loadAutogeneratedPaths(), staticPaths),
      buildResourcePaths(),
    ),
    components: {
      securitySchemes: {
        ...
        CreateShipmentRequest: {
          ...
          properties: {
            ...
            sizeTier: {
              ...
              enum: SIZE_TIER_IDS,
              description:
                "Package size band; server stores representative weight (kg) for pricing.",
            },
            ...
            packagePhotoUrl: {
              type: "string",
              format: "uri",
              description: "Optional; set after media upload (e.g. Cloudinary).",
            },
          },
        },
